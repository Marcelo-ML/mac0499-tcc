...
\posterbox[adjusted title=Introdução e motivação]{name=introbox, column=1, span=4, below=titlebox}{
  ...
  A Teoria Extremal dos Grafos estuda propriedades dos grafos sob restrições estruturais. Compreender propriedades extremais tem aplicações em análise de algoritmos, modelagem de redes e otimização.
...
\posterbox[adjusted title=Resultados principais]{name=resultbox, column=5, span=8, below=methodbox}{
    ...
    O trabalho de~\ref{EFPS} prova que $D(G) \leq \frac{4e(G)^2}{n^2}$ e $D(G) \leq \frac{n^2}{18+\epsilon}$ (para algum $\epsilon>0$). Em particular, a Conjectura 1 vale se $e(G) \geq 0.20n^2$.
    \begin{itemize}
