...
\posterbox[adjusted title=Introdução e motivação]{name=introbox, column=1, span=4, below=titlebox}{
  ...
  A Teoria Extremal dos Grafos estuda propriedades de grafos sob restrições: compreender propriedades extremais tem aplicações em análise de algoritmos, modelagem de redes e otimização.
  
  \begin{itemize}
    \item Grafos livres de triângulos não contêm ciclos de tamanho $3$.
    \item Pergunta natural: quantas arestas um grafo sem triângulos pode ter?
...
\posterbox[adjusted title=Problema e objetivos]{name=problembox, column=1, span=4, between=introbox and footerbox}{
  ...
  \begin{itemize}
    \item Quão ``distante'' pode estar um grafo sem triângulos de um bipartido?
    ...
    \item $D(G)$ mede essa distância pela remoção de um menor conjunto de arestas.
  \end{itemize}
...
\posterbox[adjusted title=Resultados principais]{name=resultbox, column=5, span=8, below=methodbox}{
    \begin{itemize}
      \item \textit{(Simonovits 1968)} Se $e(G) = \frac{n^2}{4}-m$, então $D(G) \leq m$.
        \begin{itemize}
          \item Resolve a Conjectura 1 se $e(G) \geq 0.21n^2$.
        \end{itemize}
      \item \ref{EFPS} $D(G) \leq \frac{4e(G)^2}{n^2}$ e $D(G) \leq \frac{n^2}{18+\epsilon}$
        \begin{itemize}
          \item Cota explícita sobre $D(G)$; resolve a Conjectura 1 se $e(G) \geq 0.20n^2 = \frac{n^2}{5}$.
        \end{itemize}
      \item \ref{BCL} Generalização de~\ref{EFPS} EFPS: prova a Conjectura 1 se $e(G) \geq 0.159n^2$ e outros resultados parciais usando \emph{álgebras de flag}.
      \item A Conjectura 1 pode ser resolvida quando $\delta(G) \geq \frac{4n}{11}$ (ver~\ref{CJK}).
    \end{itemize}
    ...
    \begin{figure}[H]
      ...
      \begin{subfigure}[c]{.48\textwidth}
        ...
        \caption*{Se~$\textstyle{\delta(G) \geq \frac{4n}{11}}$, então $G$ é da forma acima, e é possível demonstrar a Conjectura 1 diretamente para $G$.}
      \end{subfigure}
    \end{figure}
}


\posterbox[adjusted title=Discussão]{name=discussbox, column=5, span=5, between=resultbox and refbox}{
  \begin{itemize}
    \item As cotas conhecidas funcionam melhor para grafos densos: resultados do tipo ``a Conjectura 1 vale se $e(G) \geq \alpha n^2$'';
    \item Uso de álgebras de flag fornecem desigualdades assintóticas a partir de métodos numéricos;
    \item Condições de grau mínimo permite simplificação estrutural e estudo da Conjectura 1 para certos grafos abaixo do limiar $e(G) \geq \frac{n^2}{5}$.
  \end{itemize}
}

\posterbox[adjusted title=Conclusão]{name=conclusionbox, column=10, span=3, between=resultbox and refbox}{
  O uso de álgebras de flag e homomorfismos a partir de grafos de grau mínimo alto confirmam a Conjectura 1 em subclasses de grafos densos.
  O trabalho contribui revisando os métodos usados para avançar na Conjectura.
}
