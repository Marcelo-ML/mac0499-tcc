...
\posterbox[footerbox]{name=footerbox, above=bottom, column=1, span=12}{
    ...
    \vspace{4pt}
    \large
    Orientador: Guilherme Oliveira Mota\par
    \vspace{4pt}
    \small
    \textcolor{imesoftblue!30!white}
      {Departmento de Ciência da Computação --- Instituto de Matemática, Estatística e Ciência da Computação \\ Universidade de São Paulo}\\
    \small    
...
\posterbox[adjusted title=Introdução e motivação]{name=introbox, column=1, span=4, below=titlebox}{
  Um grafo $G$ é um par $(V,E)$, em que $V$ são os \emph{vértices} e $E$ são as \emph{arestas}, que são pares não ordenados de vértices.
  A Teoria Extremal dos Grafos é uma área que estuda propriedades dos grafos sob certas restrições e compreender propriedades extremais tem aplicações em análise de algoritmos, modelagem de redes, otimização e diversas outras áreas.
...
\posterbox[adjusted title=Problema e objetivos]{name=problembox, column=1, span=4, between=introbox and footerbox}{
  ...
  \begin{itemize}
    ...
    \item Escrevemos $D(G)$ para descrever essa distância, que é o
      tamanho do menor conjunto de arestas cuja remoção torna $G$
      bipartido.
    \end{itemize}

    Estamos interessados na seguinte conjectura, que permanece em aberto:\\
    \textbf{Conjectura 1
      (Erd\H os, 1975).} Todo grafo livre de triângulos $G$ com $n$
    vértices satisfaz $D(G) \leq \frac{n^2}{25}$.
  ...
  \begin{figure}[H]
    ...
    \caption*{O blow-up balanceado de $C_5$. Temos
      $e(G)=\frac{n^2}{5}$ e $D(G)=\frac{n^2}{25}$.}
  \end{figure}
}
...
\posterbox[adjusted title=Metodologia]{name=methodbox, column=5, span=8, below=titlebox}{
  \begin{itemize}
    \item \textbf{Álgebras de flag} (ver~\ref{BCL})\\$\rightarrow$ traduzem problemas de densidade em programas semidefinidos.
    \item Blow-ups\\$\rightarrow$ expandem vértices para conjuntos independentes mantendo estrutura.
    \item \textbf{Condições de grau mínimo} (ver~\ref{CJK})\\$\rightarrow$ permitem simplificar o problema para blow-ups de grafos pequenos.
  \end{itemize}
}


\posterbox[adjusted title=Resultados principais]{name=resultbox, column=5, span=8, below=methodbox}{
    Por resultados clássicos de estabilidade de Simonovits, é possível verificar a Conjectura 1 se $e(G) \geq 0.21n^2$.
    Em~\ref{EFPS} é provado que $D(G) \leq \frac{4e(G)^2}{n^2}$ e $D(G) \leq \frac{n^2}{18+\epsilon}$ para algum $\epsilon>0$. Em particular, a Conjectura 1 vale se $e(G) \geq 0.20n^2$.
    \begin{itemize}
      \item \ref{BCL} $\rightarrow$ Generaliza~\ref{EFPS}: prova a Conjectura 1 se $e(G) \geq 0.1599n^2$ e outros resultados parciais usando \emph{álgebras de flag}.
      \item Usando~\ref{CJK}, provamos a Conjectura 1 quando $\delta(G) > \frac{4n}{11}$.
    \end{itemize}
    ...
    \begin{figure}[H]
      \begin{subfigure}[c]{.48\textwidth}
        \centering
        \include{figuras/local-cut-efps-tikz}
        \caption*{Observe que~$\textstyle{D(G) \leq e(G-N(v))}$, pois $N(v)$ é independente.
        Na linguagem de álgebras de flag, essa figura representa o corte local relacionado à restrição $\average{\labeledCherryComplement} \geq \frac{2}{25}$.}
      \end{subfigure}
      \hfill
      \begin{subfigure}[c]{.48\textwidth}
        \centering
        \include{figuras/f4-blowup-tikz}
        \caption*{Se~$\textstyle{\delta(G) > \frac{4n}{11}}$, então $G$ é da forma acima, e é possível demonstrar a Conjectura 1 usando essa informação estrutural extra.}
      \end{subfigure}
    \end{figure}
}
...
\posterbox[adjusted title=Discussão]{name=discussbox, column=5, span=4.5, between=resultbox and refbox}{
  \begin{itemize}
    ...
    \item Condições de grau mínimo simplificam os grafos, mesmo abaixo do limiar $e(G) \geq \frac{n^2}{5}$ para provar a Conjectura 1(estrutural).
  \end{itemize}
}
