...
Fixe $\calh$ uma família de subgrafos proibidos.
Um \emph{tipo} de \emph{tamanho $k$} é um grafo $\calh$-livre $\sigma$ com $V(\sigma)=[k]$.
Denotamos o tipo vazio como $\emptyflag$.
Um \emph{$\sigma$-flag} é um par $(F,\theta)$, em que $F$ é um grafo $\calh$-livre tal que $\theta \colon [k] \to V(G)$ é injetora e define um isomorfismo entre $\sigma$ e $G[{\rm Im}(\theta)]$.
...
Note que todo grafo $G$ está naturalmente identificado com o $\emptyflag$-flag $(G,\varnothing)$.

Definimos isomorfismos entre flags da mesma forma que entre grafos, mas também preservando o isomorfismo nos vértices rotulados: dois $\sigma$-flags $(G_1,\theta_1)$ e $(G_2,\theta_2)$ são isomorfos se existe um isomorfismo $\rho \colon V(G_1) \to V(G_2)$ tal que $\rho(\theta_1(i))=\theta_2(i)$ para cada $i \in [|\sigma|]$.
Finalmente, definimos $\calf_m^\sigma$ como o conjunto de todos os $\sigma$-flags de tamanho $m$, a menos de isomorfismo e $\calf^\sigma \coloneqq \cup_{m \geq |\sigma|} \calf_m^\sigma$.

Para definir densidades, sejam $(G,\theta),F_1,F_2,\dots,F_t$ flags com
\[ |G|-|\sigma| \geq  \sum_{i=1}^t (|F_i|-|\sigma|).\]
Então definimos a densidade $d(F_1,F_2,\dots,F_t,G)$ como a probabilidade de que, ao escolher conjuntos dois a dois disjuntos $U_1,U_2,\dots,U_t \subseteq V(G) \setminus {\rm Im}(\theta)$ com $|U_i|=|F_i|-|\sigma|$, vale que o $\sigma$-flag $(G[U_i \cup {\rm Im}(\theta)],\theta)$ é isomórfico a $F_i$ para cada $i \in [t]$.
Quase sempre usamos apenas $t=1$ e $t=2$. 

O seguinte teorema permite a manipulação de densidades multiplicativas.
\begin{theorem}\label{thm:multiplying-densities}
  Para $F_1,F_2 \in \calf_m^\sigma$ e $G \in \calf^\sigma$ com $|G| \geq |F_1|+|F_2|-|\sigma|$, vale que
  \[ d(F_1,G)d(F_2,G) = d(F_1,F_2,G) + O(|G|^{-1}). \]
\end{theorem}
...
Para $F \in \calf^\sigma$, definimos $\average{F} \coloneqq q(F)\downward F$, onde $\downward F \in \calf^\emptyflag$ é uma cópia de $F$ em que os rótulos especiais são esquecidos, e $q(F)$ é a probabilidade que um homomorfismo injetor $\theta \colon [|\sigma|] \to V(\downward F)$, escolhido uniformemente ao acaso, satisfaça que $(\downward F, \theta)$ é isomórfico a $F$.
Em seguida, estendemos $\average{\cdot}$ linearmente para $\cala^\sigma$.
Por exemplo, temos
\[
...
O seguinte teorema será essencial para as aplicações futuras de álgebras de flag.
\begin{theorem}\label{thm:averaging-flag}
  Seja $F$ um $\sigma$-flag e $G$ um $\emptyflag$-flag com $|G| \geq |F|$ e $d(\downward \sigma,G) > 0$.
  Então, se $\theta$ é escolhido uniformemente ao acaso entre todas as injeções $\theta \colon [|\sigma|] \to V(G)$ com $G[{\rm Im}(\theta)]$ isomórfico a $\sigma$, vale que
  \[ \bbe_{\theta} \left[ d(F,(G,\theta)) \right] = \frac{q(F)d(\downward F, G)}{q(\sigma)d(\downward \sigma,G)}. \]
\end{theorem}
...
Vamos mostrar mais uma vez o Teorema de Mantel usando a expressividade da álgebra que acabamos de desenvolver.
...
% Para descrever o método semidefinido, precisamos listar $\calf_m^\sigma=\{F_1,F_2,\dots,F_\ell\}$.
% Para $|\sigma|$ e $m$ grandes, $\ell$ se torna maior ainda, e por isso em geral procuramos valore de $|\sigma|$ e $m$ razoáveis.
% Isso ficará mais claro a seguir.

% Tomamos $v \in \bbr^{\ell}$ um vetor e $Q \in \bbs^\ell_+$ uma matriz positiva semidefinida (ou seja, uma matriz tal que $x^\top Q x \geq 0$ para todo $x \in \bbr^{\ell}$).
% Então expandindo $v^\top Q v \geq 0$, temos
% %\[ \sum_{F_a,F_b \in \calf_m^\sigma} Q_{ab} v_a v_b \geq 0. \]
% \[ \sum_{i,j=1}^{\ell} Q_{ij}v_iv_j \geq 0. \]

% Fixe dois grafos $G$ e $H$.
% Aqui, pensamos em $G$ como o grafo ``grande'' que controla o comportamento assintótico da expressão (em particular, que controla o termo de erro) e em $H$ tem tamanho $l$ (constante) e usaremos $H$ para simplificar densidades em $G$ usando a regra da cadeia.
% Seja $\Theta_G$ o conjunto de injeções $\theta \colon [|\sigma|] \to V(G)$ tais que $(G,\theta)$ é um $\sigma$-flag.
% Defina $\Theta_H$ analogamente e fixe $\theta_G \in \Theta_G$ e $\theta_H  \in \Theta_H$.

% Fazendo $v_i = d(F_i,(G,\theta_G))$ e usando o Teorema~\ref{thm:multiplying-densities}, obtemos
% \begin{equation}\label{eq:1}
%   %\sum_{F_i,F_j \in \calf_m^\sigma} Q_{ij} d(F_i,F_j,G;\theta) + o(1) \geq 0.
%   \sum_{i,j=1}^{\ell}Q_{ij}d(F_i,F_j,(G,\theta_G)) + o(1) \geq 0.
% \end{equation}

% %Tome $H \in \calf_l^\emptyflag$, com $l$ pequeno de forma a obter uma aproximação razoável para as densidades de~(\ref{eq:1}).
% %Seja $\Theta_H$ o conjunto de injeções $\theta \colon [|\sigma|] \to V(H)$ satisfazendo que $(H,\theta)$ é um $\sigma$-flag.
% Da mesma forma que em~(\ref{eq:chain-rule-2}), temos que
% \begin{equation}\label{eq:2}
%   d(F_i,F_j,(G,\theta_G)) = \sum_{H \in \calf_l^{\emptyflag}} d(F_i,F_j,(H,\theta_H))d((H,\theta_H),(G,\theta_G)),
% \end{equation}
% e podemos calcular os coeficientes esperados $d(F_i,F_j,(H,\theta))$ quando escolhemos, de forma aleatória, o a injeção $\theta \in \Theta_H$ tal que $(H,\theta)$ é um $\sigma$-flag.

% De (\ref{eq:1}) e (\ref{eq:2}), temos então
% \[ \sum_{H \in \calf_l^{\emptyflag}} \sum_{i,j=1}^{\ell} Q_{ij} \bbe_{\theta_H \in \Theta_H} \left[d(F_i,F_j,(H,\theta_H))\right] d((H,\theta_H),(G,\theta_G)) + o(1) \geq 0. \]
