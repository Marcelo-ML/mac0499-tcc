...
\begin{theorem}\label{thm:local-cut-1}
  Se $\labeledCherryComplement \geq 2/25$ para todo flag $G^{\aSinglePoint}$ obtido de $G$ rotulando um vértice especial, então $\edge \leq 2/5$.
\end{theorem}
\begin{proof}
  ...
  logo da média sobre todas as escolhas de $G^{\sigma}$ vale que $\average{\labeledCherryComplement} \geq 2/25 \iff \unlabeledCherry \geq \frac{6}{25}$, e assim temos
  \begin{align*}
    \edge + \frac{6}{25}x 
...
Se nenhum desses cortes deixa no máximo $n^2/25$ arestas de fora, então a densidade esperada das arestas fora de qualquer um desses cortes definidos localmente é pelo menos $2/25$, o que pode ser expressado da seguinte maneira, onde as densidades são tomadas sobre um flag $G^{\sigma}$ com uma não-aresta $uv$ rotulada:

\begin{equation}\label{eq:local-cut-2}
  \frac12 \fourVerticesOne + \frac12 \fourVerticesTwo + \frac12 \fourVerticesThree \geq \frac{2}{25}.
\end{equation}
...
Escolhendo $S$ aleatoriamente e usando o Teorema~\ref{thm:averaging-flag} (supondo que $d(\downward \sigma, G) > 0$), vale que
\[ \sum_{X,Y \subseteq V(\sigma)} (p_Xp_Y+(1-p_X)(1-p_Y)) \frac{q(F^\sigma_{X,Y}) d(\downward F^\sigma_{X,Y},G)}{q(\sigma)d(\downward \sigma, G)} \geq \frac{2}{25} + o(1). \]
Omitindo $G$ e o termo $o(1)$, que já sabemos que podemos omitir, obtemos finalmente
\begin{equation}\label{eq:sigma-p-restricao}
  \sum_{X,Y \subseteq V(\sigma)} (p_Xp_Y+(1-p_X)(1-p_Y))\average{F^\sigma_{X,Y}} \geq \frac{2}{25} \average{\sigma}.
\end{equation}
Observe que essa última desigualdade vale mesmo no caso $\average{\sigma} = 0$, pois aí também teríamos $\average{F^\sigma_{X,Y}} = 0$ para cada escolha de $X$ e $Y$.
...
\[ \sum_{F \in \calf^{\emptyflag}_m} \left(\sum_{X,Y \subseteq V(\sigma)} (p_Xp_Y+(1-p_X)(1-p_Y))\left([F]\average{F^\sigma_{X,Y}}\right) - \frac{2}{25}[F]\average{\sigma}\right)F \geq 0, \]
...
\begin{equation}\label{eq:c-coeff}
  \sum_{F \in \calf^{\emptyflag}_m} c(F)F \geq 0,
\end{equation}
...
\begin{align*}
  \edge &\leq \sum_{F \in \calf^{\emptyflag}_m} \left(b(F) + \sum_{i=1}^r c_i(F) \cdot \alpha_i + \sum_{k=1}^s \sum_{i,j=1}^{\ell_k} d_{kij}(F) \cdot (A_k)_{ij}\right)F\\
  ...
  &\leq \max_{F \in \calf^{\emptyflag}_m} \left(b(F) + \sum_{i=1}^r c_i(F) \cdot \alpha_i + \sum_{k=1}^s \sum_{i,j=1}^{\ell_k} d_{kij}(F) \cdot (A_k)_{ij}\right),
\end{align*}
...
\begin{alignat*}{2}
  ...
  \text{sujeito a}
  \quad & M-\sum_{i=1}^rc_i(F) \; \alpha_i \\
...
Escolher bons valores de $\sigma$ e $p$ é um desafio porque o número de restrições geradas cresce exponencialmente, e para $|\sigma| \geq 5$ já é impraticável gerar todas as $(\sigma,p)$-restrições sem escolher ``à mão'' valores de $\sigma$ e $p$.
Em~\cite{baloghclemenlidicky}, os autores comentam pelo menos duas escolhas de $(\sigma,p)$-restrições com $|\sigma|=6$, uma delas formulada especialmente para lidar com o \emph{grafo de Clebsch}, um grafo de Cayley que é descrito como ``particularmente hostil'' com respeito à estratégia de cortes locais.

%Por exemplo, se $p_X \in \{0.0,0.5,1.0\}$ e $\sigma = \unlabeledCherryComplement$, há (sem otimizações) $3^8 \approx 6561$ cortes gerados apenas para essa escolha de $\sigma$.
%
%Duas otimizações imediatas podem ser feitas:
%\begin{enumerate}
%  \item Se $X$ não é independente em $\sigma$, então o valor de $p_X$ é irrelevante, porque $G$ é livre de triângulos.
%  No caso de $\unlabeledCherryComplement$, isso reduz o número de cortes para $3^6$ e  pode ser feito na hora de gerar as restrições que serão utilizadas.
%  \item Na hora de montar o programa, podemos rejeitar incluir novos cortes que deem desigualdades já presentes.
%  Por exemplo, em $\unlabeledCherry$ há dois vértices simétricos, e portanto é razoável esperar outra melhora por um fator constante nessa otimização.
%\end{enumerate}

Estamos desenvolvendo uma implementação do método de~\cite{baloghclemenlidicky} que pode ser encontrada em~\href{https://github.com/Marcelo-ML/flag-algebras}.
Até o momento da conclusão desse trabalho, ainda não conseguimos reproduzir os resultados do Teorema~\ref{thm:baloghclemenlidicky}.
Contudo, estamos buscando estratégias de otimização e incorporação de novas restrições locais que permitam agilizar a geração dos programas definidos correspondentes.